...
async function main() {
  ...
  await prisma.reciteLog.deleteMany();
  await prisma.recitePlan.deleteMany();
  await prisma.collectionQuote.deleteMany();
  await prisma.collection.deleteMany();
  await prisma.dailyRecommendQuote.deleteMany();
  await prisma.dailyRecommend.deleteMany();
  await prisma.quoteCategory.deleteMany();
  await prisma.originalTextCategory.deleteMany();
  await prisma.quote.deleteMany();
  await prisma.customQuote.deleteMany();
  await prisma.themePackage.deleteMany();
  await prisma.compositionTemplate.deleteMany();
  ...
  // 完整分类体系：4 大维度 + 子分类，金句和原文共用
  const categoryTree = [
    // 维度 1：内容类型 content_type
    { name: '古诗词曲', type: 'content_type', sort: 0, children: [
      { name: '唐诗', type: 'content_type' }, { name: '宋词', type: 'content_type' },
      { name: '元曲', type: 'content_type' }, { name: '诗经', type: 'content_type' },
      { name: '楚辞', type: 'content_type' }, { name: '乐府诗', type: 'content_type' },
      { name: '古风', type: 'content_type' }, { name: '五代诗词', type: 'content_type' },
      { name: '明清诗词', type: 'content_type' },
    ]},
    { name: '现代诗歌', type: 'content_type', sort: 1, children: [
      { name: '朦胧诗', type: 'content_type' }, { name: '新月派', type: 'content_type' },
      { name: '自由诗', type: 'content_type' }, { name: '散文诗', type: 'content_type' },
    ]},
    { name: '名人名言', type: 'content_type', sort: 2, children: [
      { name: '中国名人名言', type: 'content_type' }, { name: '外国名人名言', type: 'content_type' },
    ]},
    { name: '谚语俗语', type: 'content_type', sort: 3, children: [
      { name: '歇后语', type: 'content_type' }, { name: '农谚', type: 'content_type' },
      { name: '俗语', type: 'content_type' }, { name: '格言警句', type: 'content_type' },
    ]},
    { name: '影视台词', type: 'content_type', sort: 4, children: [
      { name: '电影台词', type: 'content_type' }, { name: '电视剧台词', type: 'content_type' },
      { name: '动漫台词', type: 'content_type' },
    ]},
    { name: '名著摘句', type: 'content_type', sort: 5, children: [
      { name: '中国古典名著', type: 'content_type' }, { name: '外国名著', type: 'content_type' },
    ]},
    { name: '文言名句', type: 'content_type', sort: 6 },
    { name: '歌词', type: 'content_type', sort: 7, children: [
      { name: '经典老歌', type: 'content_type' }, { name: '流行歌词', type: 'content_type' },
      { name: '民谣歌词', type: 'content_type' }, { name: '影视OST', type: 'content_type' },
    ]},
    ...
    { name: '青春成长', type: 'theme', sort: 10, children: [
      { name: '梦想', type: 'theme' }, { name: '奋斗', type: 'theme' },
      { name: '时间', type: 'theme' }, { name: '选择', type: 'theme' },
    ]},
    { name: '家国情怀', type: 'theme', sort: 11, children: [
      { name: '爱国', type: 'theme' }, { name: '乡愁', type: 'theme' },
      { name: '历史', type: 'theme' }, { name: '英雄', type: 'theme' },
    ]},
    { name: '情感共情', type: 'theme', sort: 12, children: [
      { name: '亲情', type: 'theme' }, { name: '友情', type: 'theme' },
      { name: '爱情', type: 'theme' }, { name: '离别', type: 'theme' },
    ]},
    { name: '思辨哲理', type: 'theme', sort: 13, children: [
      { name: '生命', type: 'theme' }, { name: '善恶', type: 'theme' },
      { name: '得失', type: 'theme' }, { name: '人性', type: 'theme' },
    ]},
    { name: '文化传承', type: 'theme', sort: 14, children: [
      { name: '传统', type: 'theme' }, { name: '节气', type: 'theme' },
      { name: '节日', type: 'theme' }, { name: '经典', type: 'theme' },
    ]},
    { name: '科技教育', type: 'theme', sort: 15, children: [
      { name: '知识', type: 'theme' }, { name: '读书', type: 'theme' },
      { name: '创新', type: 'theme' }, { name: '科学', type: 'theme' },
    ]},
    { name: '社会观察', type: 'theme', sort: 16, children: [
      { name: '民生', type: 'theme' }, { name: '公平', type: 'theme' },
      { name: '责任', type: 'theme' }, { name: '诚信', type: 'theme' },
    ]},
    { name: '励志奋斗', type: 'theme', sort: 17, children: [
      { name: '坚持', type: 'theme' }, { name: '勇气', type: 'theme' },
      { name: '自信', type: 'theme' }, { name: '逆境', type: 'theme' },
    ]},
    { name: '自然风光', type: 'theme', sort: 18, children: [
      { name: '山水', type: 'theme' }, { name: '田园', type: 'theme' },
      { name: '四季', type: 'theme' }, { name: '花鸟', type: 'theme' },
    ]},
  ...
  for (const root of categoryTree) {
    const parent = await prisma.category.create({
      data: { name: root.name, type: root.type, sort: root.sort ?? 0 },
    });
    categoryMap[`${root.type}:${root.name}`] = parent.id;
    if (root.children) {
      for (let j = 0; j < root.children.length; j++) {
        const child = root.children[j];
        const c = await prisma.category.create({
          data: { name: child.name, type: child.type, parentId: parent.id, sort: j },
        });
        categoryMap[`${child.type}:${child.name}`] = c.id;
      }
    }
  }
  console.log(`  ✅ 分类创建完成，共 ${Object.keys(categoryMap).length} 条`);

  console.log('🎴 创建卡片模板 CardTemplate...');
  const cardTemplates = [
    {
    ...
    },
    {
      ...
      isMember: 1, showAuthor: 0, textAlign: 'left' as const,
    },
  ];
  await prisma.cardTemplate.createMany({ data: cardTemplates });
  console.log('  ✅ 卡片模板 5 条');

  console.log('📢 创建广告配置 AdConfig...');
  const adConfigs = [
  ...
  await prisma.adConfig.createMany({ data: adConfigs });
  console.log('  ✅ 广告配置 6 条');

  console.log('💬 创建素材 Quote...');
  const quotes = [
  ...
  for (const q of quotes) {
    ...
    const quoteCategories = q.cats.map((key) => ({
      quoteId: quote.id,
      categoryId: categoryMap[key],
    }));
    await prisma.quoteCategory.createMany({ data: quoteCategories });
  }
  console.log(`  ✅ 素材 ${quotes.length} 条 + QuoteCategory 关联已建立`);

  console.log('🕷️ 创建采集源 CrawlSource...');
  const GITHUB_RAW = 'https://raw.githubusercontent.com';
  const crawlSources = [
    // ================================================================
    // 一、结构化开源数据集（最高优先级，零爬虫成本，直接批量入库）
    // ================================================================
  ...
  await prisma.crawlSource.createMany({ data: crawlSources });
  const gCount = crawlSources.filter((s) => s.type === "github").length;
  const aCount = crawlSources.filter((s) => s.type === "api").length;
  const pCount = crawlSources.filter((s) => s.complianceTag === "public_domain").length;
  console.log(`  ✅ 采集源 ${crawlSources.length} 条（GitHub直导入 ${gCount} / API ${aCount} / 公版 ${pCount}）`);
