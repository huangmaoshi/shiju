# 拾句爬虫 Python 版（crawler-py）

金句/作文素材采集爬虫，由 `crawler/`（Node.js/TypeScript）完整转换而来。
复用 `server-py` 的 SQLAlchemy 模型，连接同一个数据库（默认 `server-py/data/dev.db`），不另建数据库。
...
在 `crawler-py` 目录下执行（使用项目根的 `.venv`）：
...
crawler-py/
  crawler/
    ...
    db.py            # 数据库（sys.path 复用 server-py 模型，对应 TS db.ts）
...
- 去重：复用 `server-py/app/utils/dedup.py`（MD5 + SimHash）；重复内容更新 `CrawlRecord.auditStatus=3` 后跳过。
- 敏感词：本地词表，命中则 `auditStatus=2` 标记、不入正式 Quote 表。
- 环境变量：`CRAWL_LOG_LEVEL` / `CRAWL_CONCURRENCY` / `CRAWL_TIMEOUT` / `CRAWL_MAX_RETRY` / `CRAWL_DELAY_MIN` / `CRAWL_DELAY_MAX`，读取 `crawler-py/.env`。
